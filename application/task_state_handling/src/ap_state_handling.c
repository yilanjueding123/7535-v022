...
{
	if (((cmd1 < ICON_MP3_PLAY) || (cmd1 > ICON_MP3_STOP)) && ((cmd2 < ICON_MP3_PLAY) || (cmd2 > ICON_MP3_STOP)) &&
		 ((cmd3 < ICON_MP3_PLAY) || (cmd3 > ICON_MP3_STOP)))
	{
		OSQPost(DisplayTaskQ, (void *) (MSG_DISPLAY_TASK_ICON_SHOW | (cmd1 | (cmd2 << 8) | (cmd3 << 16))));
	}
	else 
	{
		if (((present_state == STATE_VIDEO_PREVIEW) || (present_state == STATE_VIDEO_RECORD)) /* && (fm_tx_status_get(
			) == 1)*/)
		{
			OSQPost(DisplayTaskQ, (void *) (MSG_DISPLAY_TASK_ICON_SHOW | (cmd1 | (cmd2 << 8) | (cmd3 << 16))));
		}
...
INT32S ap_state_handling_jpeg_decode(STOR_SERV_PLAYINFO * info_ptr, INT32U jpg_output_addr)
{
	...
	{ // TV, RGB565
		...
		if (info_ptr->file_type == TK_IMAGE_TYPE_JPEG)
			status =
				 video_decode_jpeg_as_rgb565_by_piece(info_ptr->file_handle, size, TV_WIDTH, TV_HEIGHT, jpg_output_addr);
		else 
	...
	{ // DISP_DEV_TFT, RGB565
		...
		if (info_ptr->file_type == TK_IMAGE_TYPE_JPEG)
			status =
				 video_decode_jpeg_as_rgb565_by_piece(info_ptr->file_handle, size, TFT_WIDTH, TFT_HEIGHT, jpg_output_addr);
		else 
...
INT32S scaler_set_parameters(void)
{
	...
	{
		scaler_out_buffer	= (INT32U)
		gp_malloc((jpeg_output_buffer_width * jpeg_output_buffer_height) << 1);
	...
	else 
	{
		...
		if (jpeg_output_ratio == 0x0)
		{ // Fit to output buffer width and height
			goto __exit;
		}
		else if (jpeg_output_ratio == 2 && jpeg_valid_width <= jpeg_output_image_width &&
			 jpeg_valid_height <= jpeg_output_image_height)
		{
...
INT32S scaler_fifo_once(void)
{
	...
	if (jpeg_output_h_cnt >= jpeg_valid_height)
	{
		...
		if (jpeg_image_yuv_mode == C_JPG_CTRL_YUV422 || jpeg_image_yuv_mode == C_JPG_CTRL_YUV422V)
		{
...
INT32S jpeg_buffer_decode_and_scale(IMAGE_DECODE_STRUCT * img_decode_struct)
{
	...
	parse_status		=
		 jpeg_decode_parse_header((INT8U *) img_decode_struct->image_source, img_decode_struct->source_size);
	...
	p_vlc				= (INT8U *)
	jpeg_decode_image_vlc_addr_get();
...
void jpeg_memory_allocate(INT32U fifo)
{
	...
	jpeg_out_y			= (INT32U)
	gp_malloc_align(jpeg_output_y_size, 32);
	...
	{
		jpeg_out_cb 		= (INT32U)
		gp_malloc_align(jpeg_output_cb_cr_size, 8);
